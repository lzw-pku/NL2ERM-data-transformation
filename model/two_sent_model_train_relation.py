...
class RelRelationClassifierBaseline(nn.Module):
    def __init__(self, type_emb_size=512, hidden_size=1024, label_num=3, tokenizer=None, model_type='base'):
        ...
        self.start_index = tokenizer.convert_tokens_to_ids(['[CLS]'])[0]
        self.end_index = tokenizer.convert_tokens_to_ids(['[SEP]'])[0]
        self.encoder = BertModel.from_pretrained('bert-{}-uncased'.format(model_type),add_pooling_layer=False)

    def forward(self, sent1, span_start1, type1, length1,
                sent2, span_start2, type2, length2, relation=None):
        ret = []
        for s1, t1, s2, t2, in zip(sent1, type1, sent2, type2):
            if t1 == t2:
                tmp1 = []
                flag = False
                for x in s1:
...
class RelationClassifier(nn.Module):
    ...
    def forward(self, sent1, span_start1, type1, length1,
                sent2, span_start2, type2, length2, relation=None):
        span_emb1 = self.get_span_emb(sent1, span_start1, type1, length1) # bs * (hid+emb)
        span_emb2 = self.get_span_emb(sent2, span_start2, type2, length2) # bs * (hid+emb)
        #merged_emb = torch.cat([span_emb1, span_emb2], dim=-1) # bs * (2*(hid+emb))
        merged_emb = torch.cat([span_emb1 - span_emb2, span_emb1 + span_emb2], dim=-1)
        feature = self.transformer(merged_emb) # bs * hid
        feature = self.trans2(feature)
        logits = self.classifier(feature) # bs * num_label
        #print(logits) 
    ...
    def predict(self, sent1, span_start1, type1,
                sent2, span_start2, type2, return_logit=False):
            sent1 = sent1.unsqueeze(0)
            sent2 = sent2.unsqueeze(0)
            span_start1 = torch.tensor([span_start1], dtype=torch.int64, device=sent1.device)
            span_start2 = torch.tensor([span_start2], dtype=torch.int64, device=sent2.device)
            type1 = torch.tensor([type1], dtype=torch.int64, device=sent1.device)
            type2 = torch.tensor([type2], dtype=torch.int64, device=sent1.device)
            length1 = torch.tensor([sent1.size(1)], dtype=torch.int64, device=sent1.device)
            length2 = torch.tensor([sent2.size(1)], dtype=torch.int64, device=sent1.device)
            logits = self.forward(sent1, span_start1, type1, length1,
                                  sent2, span_start2, type2, length2)
            #print(logits)
            #print(logits.size())
            if return_logit:
...
class PipeLine(nn.Module):
    ...
    def forward(self, sents, lengths):
        ...
        '''
        for i, (s, l) in enumerate(zip(sents, lengths)):
            s_emb, t_pred, c_pred = self.extractor.predict(s.unsqueeze(0),
                                                           l.unsqueeze(0))
            table_spans = label2span(t_pred, i, 0)
            column_spans = label2span(c_pred, i, 1)
            concept_list += table_spans
        ...
        label_map = {c1:{c2: torch.tensor([0., 1., 0.]) for c2 in concept_list}
        ...
        for i in range(concept_num):
            for j in range(i + 1, concept_num):
                ...
                label_map[span1][span2] = torch.tensor(pred)
                label_map[span2][span1] = torch.tensor(pred)
        #import copy
        #old_map = copy.deepcopy(label_map)
        #old_concept_list = copy.deepcopy(concept_list)
        import pickle
        with open('./inter_pred.pkl', 'wb') as file:
            pickle.dump((concept_list, label_map), file)
        self.aggregate(concept_list, label_map)
        return concept_list, label_map

    def aggregate(self, concept_list, label_map):
            ...
            for i in range(concept_num):
                for j in range(i + 1, concept_num):
                    ...
                    for x in concept_list:
                        tmp.append((label_map[span1][x] * label_map[span2][x]).sum())
                    tmp = torch.stack(tmp)
                    tmp = tmp.mean()
                    print(tmp)
                    print(tmp * (1-alpha))
                    print(label_map[span1][span2][1])
                    '''
